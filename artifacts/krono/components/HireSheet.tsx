...
function QrcodeContent({
  ...
  const handleBarcodeScanned = useCallback(
    async ({ data }: { type: string; data: string }) => {
      ...
      // ── Try Krono QR payload (primary path) ──────────────────────────────
      try {
        const parsed = JSON.parse(data) as QRPayload;
        if (parsed.type === "krono-qr" && parsed.v === 1 && parsed.pid && parsed.sid && parsed.pin && parsed.chk) {
          setValidating(true);
          try {
        ...
        // not a Krono JSON payload — fall through to PIN fallback
      }
      ...
      if (/^\d{4,6}$/.test(data.trim())) {
        ...
        onShowDialog({
          title: "QR Code inválido",
          message: "Este QR Code não é reconhecido pelo Krono.",
        });
        setTimeout(() => setScanned(false), 3000);
      }
    },
